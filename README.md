...
**1%의 정확도 향상과 1ms의 지연 시간 단축에 집착합니다**
...
`FastAPI` · `Next.js 16` · `Celery/Redis` · `Playwright` · `Lighthouse` · `LLM API` · `PostgreSQL`

**커밋 338 · 머지 147 · 테스트 557** — [개발일지](https://shin-h-s.github.io/devlog.html)

> 🚧 **지금은 조사 에이전트를 개발 중** — 장애가 감지되면 도구 루프로 검사 결과를 직접 조사해
> 원인을 7유형으로 분류하고 조치를 제안하는 LLM 에이전트. 평가부터 먼저 — 실측 사고를 포함한
> 평가셋 175건(dev/test 분리)과 코드 채점기를 완성해 목표선을 고정했고, 운영 연결까지 마쳐
> **실제 장애를 자동 조사해 원인을 정확히 판정하고 Discord로 보고하는 것까지 검증**했습니다.
